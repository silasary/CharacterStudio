...
namespace BuildTools
{
    static class GenerateSpecificsEnum
    {
        ...
        private static void ScanOldSource(Dictionary<string, string> Purposes)
        {
            if (!File.Exists("Specifics.cs"))
                return;
            var Summary = new Regex("<summary>(.*)</summary>");
            var Name = new Regex("^([A-Za-z_]+),");
        ...
        private static void EmitSpecificsSource(Dictionary<string, IEnumerable<string>> Specifics, Dictionary<string, string> Purposes, StringBuilder ClassWriter)
        {
            ClassWriter.Append(
@"/*
...
");
            foreach (var s in Specifics)
            {
                ...
                if (HasPurpose)
                    ClassWriter.AppendLine(string.Format(@"/// <summary>{0}</summary>", Purposes[s.Key]));
                else
                {
